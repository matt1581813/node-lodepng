...
void encode_execute(napi_env env, void* _data) {
  ...
  }else if (data->inputLength == data->width * data->height * 3) {
      data->error = lodepng_encode24(&data->output, &data->outputLength, data->input, data->width, data->height);
  }
  else {
...
napi_value encodeSync(napi_env env, napi_callback_info info) {
    size_t argc = 3;
    napi_value args[3];
  ...
  //  assert(napi_create_reference(env, args[0], 1, &data->reference) == napi_ok);
   ...
   //         free(data->input);
    ...
    napi_create_buffer_copy(env, data->outputLength, data->output, NULL, &buffer);
...
napi_value encode(napi_env env, napi_callback_info info) {
  ...
  if (data->inputLength != data->width * data->height * 4 && data->inputLength != data->width * data->height * 3) {
      napi_value error;
